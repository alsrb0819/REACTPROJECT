...
export const START_PROJECT = 'common/dashboard/reducer_startPrj';   // 리듀서에 액션을 등록할 때 store내에 모든 리듀서에 등록이 되며 겹치면 안되기에 경로를 적어주는 것이다.
...
const initialState = {
    startPrj: sessionStorage.getItem('dashboard_off'),  // sessionStorage는 사용자가 만지는 브라우저
    status: 'INIT',
    empInfo: [],
    isLogin: false,
    errorCode: '',
...
export const dashReducer = (state = initialState, action) => {
    switch (action.type) {
        case START_PROJECT:
            sessionStorage.setItem('dashboard_off', true);  // 사작버튼을 누르면 false가 ture로 바뀐다.
            return {
                ...state,
